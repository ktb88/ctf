...
FLAG = "TDCTF{super_hero_doesnt_need_any_help}"
...
def load_maps():
	...
	for i in range(0, 1001):
		hash_msg = "%d%d" % (i, salt)
		hash_md5 = hashlib.md5(hash_msg).hexdigest()
		hash_maps[hash_md5] = i

		file_name = "/home/onmyway/maps/%04d.map" % (i)
		dict_name = "%04d" % (i)
	...
	for cmd in ["U", "L", "D", "R", "Q"]:
		hash_msg = "%c%d" % (cmd, salt)
		hash_md5 = hashlib.md5(hash_msg).hexdigest()
		hash_maps[hash_md5] = cmd
...
def game_start():
	...
	while 1:
		...
		ws(p32(0xdeadface) + p32(current_level) + p32(p_x) + p32(p_y) + show_data + "\n")
		data = raw_input()
		len_data = len(data)
		for i in range(0, len_data):
			if data[i:i+0x20] in hash_maps:
				data = data[i:]
				break
		...
		hash_cmd = data[0:0x20]
		hash_level = data[0x20:0x40]
		hash_x = data[0x40:0x60]
		hash_y = data[0x60:0x80]
		cmd = None
		level = None
		temp_x = None
		temp_y = None
		...
		if maps[map_name]["map"][p_y][p_x] == "G":
			current_level += 1
			ws("Yep! Go to next floor\n")
			p_y = -1
			p_x = -1

			if current_level > 1001 or current_level < 0:
				ws("I think you are cheeter. GET OUT! :(\n")
				exit(-1)

			if current_level == 1001:
